<?php
/**
 * Keyword × area — high contrast, unique keyword body + local SEO.
 * Placeholders: KEYWORD_*, SERVICE_*, AREA*, MANUFACTURER_TAGS, KEYWORD_INTRO, KEYWORD_BODY,
 * KEYWORD_META, KEYWORD_FOCUS_HTML, KEYWORD_IMAGE, SERVICE_IMAGE
 */
...
<section class="relative overflow-hidden bg-[#061828] text-white">
    <div class="absolute inset-0">
        <img src="{{KEYWORD_IMAGE}}" alt="" class="w-full h-full object-cover opacity-30" loading="eager"
    ...
    <div class="relative max-w-7xl mx-auto px-6 py-12 md:py-16">
        <nav class="text-xs text-white/70 mb-4 flex flex-wrap gap-2">
            ...
            <a href="<?= url('/pages/keywords/{{KEYWORD_SLUG}}.php') ?>" class="hover:text-white">{{KEYWORD_NAME}}</a><span>/</span>
            <span class="text-white font-semibold">{{AREA}}</span>
        </nav>
        ...
        <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-white leading-[1.08] drop-shadow-lg">
            {{KEYWORD_NAME}}<br><span class="text-[#ff6b00]">in {{AREA}}</span>
        </h1>
        <p class="mt-5 text-lg text-white font-medium max-w-2xl leading-relaxed drop-shadow">
            Local engineers for <strong>{{KEYWORD_NAME}}</strong> in <strong>{{AREA}}</strong> and nearby postcodes.
    ...
    <div class="max-w-7xl mx-auto px-6 py-12 md:py-16 grid lg:grid-cols-5 gap-10">
        <div class="lg:col-span-3">
            <div class="bg-white border-2 border-zinc-300 rounded-3xl p-6 md:p-8 shadow-sm">
                <h2 class="text-2xl font-bold text-[#061828]">{{KEYWORD_NAME}} for properties in {{AREA}}</h2>
                <p class="mt-4 text-lg text-zinc-900 leading-relaxed font-medium">{{KEYWORD_INTRO}}</p>
                <p class="mt-4 text-lg text-zinc-900 leading-relaxed">
                    Serving <strong class="text-[#061828]">{{AREA}}</strong>: {{KEYWORD_BODY}}
                </p>
                <p class="mt-4 text-base text-zinc-900 leading-relaxed">
                    Searching for <strong>{{KEYWORD_NAME}} near {{AREA}}</strong>? Book Icomply for install, service, testing or certification.
                    Also see
                    <a class="font-bold text-[#ff6b00] hover:underline" href="<?= url('/pages/keywords/{{RELATED_SLUG}}/{{AREA_SLUG}}.php') ?>">{{RELATED_NAME}} in {{AREA}}</a>
                    and
                    <a class="font-bold text-[#ff6b00] hover:underline" href="<?= url('/pages/{{SERVICE_SLUG}}/{{AREA_SLUG}}.php') ?>">{{SERVICE_NAME}} in {{AREA}}</a>.
                </p>
            ...
            <div class="mt-6 grid sm:grid-cols-3 gap-4">
                ...
                <div class="p-5 bg-white border-2 border-zinc-300 rounded-2xl shadow-sm">
                    <div class="font-bold text-[#061828] text-lg"><?= $t ?></div>
                    <p class="text-sm text-zinc-800 mt-1 font-medium"><?= $d ?></p>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
        <div class="lg:col-span-2 space-y-4">
            <div class="rounded-3xl overflow-hidden border-2 border-zinc-400 shadow-md">
                <img src="{{KEYWORD_IMAGE}}" alt="{{KEYWORD_NAME}} in {{AREA}}" class="w-full h-48 object-cover" loading="eager"
                     onerror="this.src='{{SERVICE_IMAGE}}'">
                <div class="bg-[#061828] text-white p-3 text-center font-bold text-sm">{{KEYWORD_NAME}} · {{AREA}}</div>
            </div>
            <div class="bg-[#061828] text-white rounded-3xl p-6 shadow-lg">
                <h3 class="text-xl font-bold">Quote — {{AREA}}</h3>
                ...
                <a href="<?= url('/pages/areas/{{AREA_SLUG}}.php') ?>" class="block mt-3 text-sm font-semibold text-[#ff6b00] hover:underline">All services in {{AREA}} →</a>
            </div>
...
<section class="bg-white border-y-2 border-zinc-200">
    <div class="max-w-7xl mx-auto px-6 py-12">
        <h2 class="text-xl font-bold text-[#061828]">{{KEYWORD_NAME}} nearby</h2>
        <div class="mt-4 flex flex-wrap gap-2">
            ...
            foreach ($towns as $t):
            ?>
            <a href="<?= url('/pages/keywords/{{KEYWORD_SLUG}}/' . areaSlug($t) . '.php') ?>"
               class="px-4 py-2 bg-[#061828] text-white rounded-full text-sm font-semibold hover:bg-[#ff6b00]">
                in <?= htmlspecialchars($t, ENT_QUOTES, 'UTF-8') ?>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
...
<section id="quote" class="bg-zinc-100 border-t-2 border-zinc-300">
    <div class="max-w-3xl mx-auto px-6 py-14">
        <h2 class="text-3xl font-bold text-[#061828] text-center">{{KEYWORD_NAME}} in {{AREA}}</h2>
        <form action="<?= url('/contact.php') ?>" method="POST" class="mt-8 bg-white border-2 border-zinc-300 rounded-3xl p-6 md:p-8 space-y-4 shadow-md">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <select name="service" required class="w-full border-2 border-zinc-300 px-4 py-3 rounded-xl bg-white font-medium text-zinc-900">
                    <option value="<?= htmlspecialchars($keywordName . ' in ' . $areaName, ENT_QUOTES, 'UTF-8') ?>" selected><?= htmlspecialchars($keywordName . ' in ' . $areaName, ENT_QUOTES, 'UTF-8') ?></option>
            ...
            <textarea name="message" rows="4" required placeholder="{{AREA}} postcode, property type, brand…" class="w-full border-2 border-zinc-300 px-4 py-3 rounded-xl font-medium text-zinc-900"></textarea>
