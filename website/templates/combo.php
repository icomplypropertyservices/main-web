...
$pageTitle = '{{SERVICE_NAME}} in {{AREA}} | UK Install & Cert';
...
<section class="relative text-white py-16 md:py-20 overflow-hidden">
    <img src="assets/images/services/{{SERVICE_SLUG}}-photo.jpg"
         alt="{{SERVICE_NAME}} in {{AREA}} — UK property compliance engineers"
         class="absolute inset-0 w-full h-full object-cover" width="1600" height="700" fetchpriority="high"
         onerror="this.src='assets/images/services/{{SERVICE_SLUG}}.png'">
    <div class="absolute inset-0 hero-overlay"></div>
    <div class="relative max-w-5xl mx-auto px-6">
        <div class="text-white/90 text-sm mb-4"><?= render_breadcrumbs($crumbs) ?></div>
        <p class="text-xs uppercase tracking-[.2em] text-[#ff6b00] font-semibold mb-3">{{SERVICE_NAME}} · {{AREA}} · <?= htmlspecialchars($profile['region']) ?></p>
        <h1 class="text-4xl md:text-5xl font-extrabold tracking-tight">{{SERVICE_NAME}} in {{AREA}}</h1>
        <p class="mt-4 max-w-2xl text-lg text-white/90">Install · service · test · certificate for landlords, agents and businesses in {{AREA}} (<?= htmlspecialchars($profile['districts']) ?>).</p>
        <div class="mt-8 flex flex-wrap gap-3">
            <a href="contact.php" class="accent-btn px-8 py-3 rounded-2xl font-semibold">Free {{AREA}} quote</a>
            <a href="tel:<?= PHONE ?>" class="px-8 py-3 rounded-2xl bg-white text-[#0a2540] font-semibold"><?= PHONE ?></a>
            <a href="https://wa.me/<?= WHATSAPP ?>?text=Quote%20for%20{{SERVICE_NAME}}%20in%20{{AREA}}" class="px-8 py-3 rounded-2xl border border-white/40 font-semibold hover:bg-white/10">WhatsApp</a>
        </div>
    </div>
</section>
...
<section class="max-w-5xl mx-auto px-6 py-14">
    <article itemscope itemtype="https://schema.org/Service">
        <meta itemprop="name" content="{{SERVICE_NAME}} in {{AREA}}">
        <meta itemprop="serviceType" content="{{SERVICE_NAME}}">

        <h2 class="text-3xl font-extrabold tracking-tight">{{SERVICE_NAME}} specialists covering {{AREA}}</h2>
        ...
        <div class="mt-8 flex flex-wrap gap-2" aria-label="Standards and keywords">
            ...
            <?php endforeach; ?>
            <span class="px-3 py-1.5 rounded-full bg-[#ff6b00]/10 text-[#ff6b00] text-xs font-semibold">{{AREA}}</span>
        ...
        <h2 class="mt-14 text-2xl font-extrabold">{{SERVICE_NAME}} for {{AREA}} property types</h2>
        ...
        <h2 class="mt-14 text-2xl font-extrabold">{{SERVICE_NAME}} installation, service and certification in {{AREA}}</h2>
        <div class="mt-6 grid md:grid-cols-3 gap-6">
            <div class="p-7 bg-white rounded-3xl border">
                <h3 class="font-bold text-lg mb-2">Installation in {{AREA}}</h3>
                <p class="text-sm text-zinc-600 leading-relaxed">Design, supply and install matched to your fire strategy, electrical design or security brief for {{AREA}} homes and commercial sites.</p>
            </div>
            <div class="p-7 bg-white rounded-3xl border">
                <h3 class="font-bold text-lg mb-2">Maintenance in {{AREA}}</h3>
                <p class="text-sm text-zinc-600 leading-relaxed">Planned visits and reactive repairs so {{AREA}} systems stay reliable between formal inspections and insurer reviews.</p>
            </div>
            <div class="p-7 bg-white rounded-3xl border">
                <h3 class="font-bold text-lg mb-2">Testing &amp; certificates</h3>
                <p class="text-sm text-zinc-600 leading-relaxed">Documented results and certificates landlords, freeholders and insurers expect for {{SERVICE_NAME}} in {{AREA}}.</p>
            </div>
        </div>

        <h2 class="mt-14 text-2xl font-extrabold">Why {{AREA}} clients choose Icomply</h2>
        ...
        <h2 class="mt-14 text-2xl font-extrabold">How booking {{SERVICE_NAME}} in {{AREA}} works</h2>
        ...
        <div class="mt-14 grid md:grid-cols-2 gap-8 items-center bg-white border rounded-3xl overflow-hidden">
            <img src="assets/images/services/{{SERVICE_SLUG}}.png"
                 alt="{{SERVICE_NAME}} compliance package for {{AREA}} UK properties"
                 class="w-full h-64 object-contain bg-zinc-50 p-8" loading="lazy" width="600" height="400">
            <div class="p-8">
                <h2 class="text-2xl font-extrabold">Who we help in {{AREA}}</h2>
                <p class="mt-3 text-zinc-600 text-sm leading-relaxed">Landlords, letting agents, RTM companies, care providers, retailers, offices, industrial units and public-sector sites needing dependable {{SERVICE_NAME}} across <?= htmlspecialchars($profile['districts']) ?> with clear paperwork and local attendance.</p>
                <p class="mt-3 text-zinc-600 text-sm leading-relaxed">Tell us storeys, occupancy, existing equipment and access windows — we scope {{SERVICE_NAME}} for real {{AREA}} conditions, not a generic checklist alone.</p>
                <a href="contact.php" class="inline-block mt-6 accent-btn px-6 py-3 rounded-xl font-semibold">Request {{AREA}} quote</a>
            </div>
        </div>

        <h2 class="mt-14 text-2xl font-extrabold">Compliance notes for {{AREA}} managers</h2>
        <p class="mt-3 text-zinc-700 leading-relaxed text-sm md:text-base">Keep a simple evidence pack for each {{AREA}} site: latest certificates, service sheets, device lists and any remedial schedule. When insurers, freeholders or local authority officers ask for proof of {{SERVICE_NAME}}, organised documents reduce delays. If you manage multiple units in <?= htmlspecialchars($profile['region']) ?>, we can align visit cycles so related systems (for example fire alarms and emergency lighting) are serviced efficiently.</p>

        <?= render_faq_section($faqs, "FAQs: {$serviceName} in {$areaName}") ?>

        <div class="mt-14 bg-zinc-900 text-white p-10 rounded-3xl">
            <h2 class="text-2xl font-bold mb-3">Book {{SERVICE_NAME}} in {{AREA}} today</h2>
            <p class="text-white/80 mb-6">Speak to a UK compliance specialist. Travel to {{AREA}} is <?= htmlspecialchars($profile['travel']) ?>.</p>
            <div class="flex flex-wrap gap-3">
                ...
                <a href="https://wa.me/<?= WHATSAPP ?>?text=Quote%20for%20{{SERVICE_NAME}}%20in%20{{AREA}}" class="inline-block border border-white/40 px-8 py-3 rounded-2xl font-semibold">WhatsApp</a>
            </div>
    ...
    <div class="mt-16">
        <h2 class="font-bold text-xl mb-4">Other compliance services in {{AREA}}</h2>
                ...
                <a href="pages/<?= htmlspecialchars($sSlug) ?>/<?= areaSlug($areaName) ?>.php" class="service-card bg-white border rounded-2xl overflow-hidden group">
                    <div class="h-28 overflow-hidden">
                        <img src="assets/images/services/<?= htmlspecialchars($sSlug) ?>-photo.jpg"
                             alt="<?= htmlspecialchars($sName) ?> in {{AREA}}"
                    ...
                    <div class="p-4 font-semibold text-sm"><?= htmlspecialchars($sName) ?> in {{AREA}}</div>
                </a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="mt-12">
        <h2 class="font-bold text-xl mb-4">{{SERVICE_NAME}} near {{AREA}}</h2>
        <div class="flex flex-wrap gap-2 text-sm">
            <?php foreach ($nearby as $a): ?>
                <a href="pages/{{SERVICE_SLUG}}/<?= areaSlug($a) ?>.php" class="px-4 py-1.5 bg-white border rounded-full hover:border-[#ff6b00]"><?= htmlspecialchars($a) ?> {{SERVICE_NAME}}</a>
            <?php endforeach; ?>
        </div>
        <p class="mt-6 text-sm text-zinc-500">
            <a class="text-[#ff6b00] font-semibold hover:underline" href="pages/services/{{SERVICE_SLUG}}.php">All {{SERVICE_NAME}} services</a>
            · <a class="text-[#ff6b00] font-semibold hover:underline" href="pages/areas/<?= areaSlug($areaName) ?>.php">All compliance in {{AREA}}</a>
