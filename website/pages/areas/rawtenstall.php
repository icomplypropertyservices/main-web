...
$pageTitle = 'Rawtenstall Property Compliance | EICR & Fire';
$metaDesc = 'Rawtenstall (' . $profile['districts'] . ') compliance: EICR, fire, gas, emergency lighting, CCTV. Free quote from Stockport engineers.';
if (mb_strlen($metaDesc) > 158) {
    $metaDesc = 'Rawtenstall property compliance: EICR, fire alarms, gas, emergency lighting & CCTV. North West engineers. Free quote.';
}
$metaKeywords = 'Rawtenstall electrician, Rawtenstall fire alarm, Rawtenstall EICR, Rawtenstall gas safety, property compliance Rawtenstall';
...
<section class="relative text-white py-16 md:py-20 overflow-hidden">
    <img src="assets/images/heroes/manchester.jpg" alt="Property compliance in Rawtenstall, <?= htmlspecialchars($profile['region']) ?> UK" class="absolute inset-0 w-full h-full object-cover" width="1600" height="700" fetchpriority="high">
    ...
    <div class="relative max-w-5xl mx-auto px-6">
        ...
        <h1 class="text-4xl md:text-5xl font-extrabold tracking-tight">Rawtenstall property compliance</h1>
        <p class="mt-4 max-w-2xl text-lg text-white/90">Local UK engineers for Rawtenstall — electrical, fire, gas, emergency lighting, security and more. Certificates landlords and insurers accept.</p>
        <div class="mt-8 flex flex-wrap gap-3">
            <a href="contact.php" class="accent-btn px-8 py-3 rounded-2xl font-semibold">Get a Rawtenstall quote</a>
...
<section class="max-w-5xl mx-auto px-6 py-14">
    <h2 class="text-3xl font-extrabold tracking-tight">Compliance services for Rawtenstall properties</h2>
    <p class="mt-4 text-zinc-700 leading-relaxed">Icomply Property Services supports landlords, managing agents and businesses in Rawtenstall (<?= htmlspecialchars($profile['districts']) ?>). Local stock typically includes <?= htmlspecialchars($profile['stock']) ?>. Travel from Stockport is <?= htmlspecialchars($profile['travel']) ?>. Focus areas: <?= htmlspecialchars($profile['focus']) ?>.</p>
        ...
        <?php foreach ($GLOBALS['services'] as $sSlug => $sName): ?>
            <a href="pages/<?= htmlspecialchars($sSlug) ?>/<?= areaSlug($areaName) ?>.php" class="service-card bg-white border rounded-2xl overflow-hidden flex group">
                <div class="w-28 shrink-0 overflow-hidden">
                    <img src="assets/images/services/<?= htmlspecialchars($sSlug) ?>-photo.jpg" alt="<?= htmlspecialchars($sName) ?> in Rawtenstall" class="img-cover h-full min-h-[7rem] group-hover:scale-105 transition duration-500" loading="lazy" width="160" height="120" onerror="this.src='assets/images/services/<?= htmlspecialchars($sSlug) ?>.png'">
                </div>
                <div class="p-5 flex-1 flex flex-col justify-center">
                    <span class="font-bold"><?= htmlspecialchars($sName) ?> in Rawtenstall</span>
    ...
    <div class="mt-14 bg-white border rounded-3xl p-8">
        <h2 class="text-xl font-extrabold">Why book Icomply in Rawtenstall?</h2>
    ...
    <div class="mt-12 text-center">
        <a href="https://wa.me/<?= WHATSAPP ?>?text=Quote%20for%20Rawtenstall" class="inline-block px-10 py-4 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-semibold">WhatsApp for a Rawtenstall quote</a>
...
<?= json_encode([
    ...
    '@type' => 'Service',
    'name' => 'Property compliance services in Rawtenstall',
    'description' => $metaDesc,
    'provider' => ['@id' => site_url() . '#business'],
    'areaServed' => ['@type' => 'City', 'name' => 'Rawtenstall'],
