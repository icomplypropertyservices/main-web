...
$pageTitle = 'Intercoms North West | Install & Service';
$metaDesc = 'Intercoms across Greater Manchester & North West UK. Install, service & certification. Stockport engineers. Free quote.';
$metaKeywords = 'intercom installation, video intercom, audio intercom, multi tenant intercom, Intercoms Manchester, Intercoms Stockport, Intercoms North West';
...
<section class="relative text-white py-16 md:py-20 overflow-hidden">
    <img src="assets/images/services/intercoms-photo.jpg" alt="Intercoms services across Greater Manchester and North West UK" class="absolute inset-0 w-full h-full object-cover" width="1600" height="700" fetchpriority="high" onerror="this.src='assets/images/services/intercoms.png'">
    <div class="absolute inset-0 hero-overlay"></div>
    <div class="relative max-w-5xl mx-auto px-6">
        <?= render_breadcrumbs($crumbs) ?>
        <div class="text-sm uppercase tracking-[3px] text-[#ff6b00] mb-3">UK PROPERTY COMPLIANCE · NORTH WEST</div>
        <h1 class="text-4xl md:text-6xl font-extrabold tracking-tight">Intercoms services</h1>
        <p class="mt-4 max-w-2xl text-lg text-white/90">Expert Intercoms installation, servicing, maintenance and certification for landlords, agents and businesses across Manchester, Stockport, Bolton, Liverpool, Preston and 140+ North West towns.</p>
...
<section class="max-w-5xl mx-auto px-6 py-14">
    <h2 class="text-3xl font-extrabold tracking-tight">What our Intercoms service includes</h2>
    ...
    <p class="mt-3 text-zinc-700 leading-relaxed">From Manchester and Stockport to Liverpool, Preston, Blackpool, Chester and Warrington, we schedule Intercoms around access windows that work for landlords and FM teams. Standards we align to include <?= htmlspecialchars(implode(', ', service_standards($serviceSlug))) ?>.</p>
    ...
    <div class="mt-14 rounded-3xl overflow-hidden border grid md:grid-cols-2 bg-white">
        <img src="assets/images/services/intercoms.png" alt="Intercoms UK compliance package" class="w-full h-64 md:h-full object-contain bg-zinc-50 p-8" loading="lazy" width="600" height="400">
        <div class="p-8 md:p-10 flex flex-col justify-center">
            ...
            <a href="contact.php" class="inline-block mt-6 modern-btn px-6 py-3 rounded-xl font-semibold w-max">Book Intercoms</a>
        </div>
    </div>
    ...
    <div class="mt-16" id="areas">
        <h2 class="font-extrabold text-2xl mb-2">Intercoms by town</h2>
        <p class="text-zinc-600 text-sm mb-5">Jump to a local landing page optimised for Intercoms in that area.</p>
        <div class="flex flex-wrap gap-2 text-sm">
            <?php foreach ($topAreas as $a): ?>
                <a href="pages/intercoms/<?= areaSlug($a) ?>.php" class="px-4 py-1.5 bg-white border rounded-full hover:border-[#ff6b00]"><?= htmlspecialchars($a) ?></a>
            <?php endforeach; ?>
        </div>
    </div>

    <div class="mt-14 bg-zinc-900 text-white p-10 rounded-3xl text-center">
        <h2 class="text-2xl font-bold">Need Intercoms this week?</h2>
