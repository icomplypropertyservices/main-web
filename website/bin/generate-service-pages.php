...
<?php
/**
 * Generates /pages/services/{slug}.php from templates/service.php for every service.
 */
require_once __DIR__ . '/../config.php';

$allServices = array_merge($services, loadServices());
$tpl = file_get_contents(__DIR__ . '/../templates/service.php');
$dir = __DIR__ . '/../pages/services';
...
echo "  [index] All services page\n";

$n = 0;
foreach ($allServices as $sSlug => $sName) {
    $out = str_replace(
        ['{{SERVICE_NAME}}', '{{SERVICE_SLUG}}', '{{SEO_KEYWORDS}}'],
        [$sName, $sSlug, getSeoKeywords($sSlug)],
        $tpl
    );
    file_put_contents("{$dir}/{$sSlug}.php", $out);
    echo "  [{$sSlug}] {$sName}\n";
    $n++;
}
echo "\nGenerated {$n} service pages + index.\n";
